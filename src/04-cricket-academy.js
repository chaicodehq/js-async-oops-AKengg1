...
export class Batsman extends Player {
  ...
  playInnings(runs, balls) {
    ...
    const obj = { runs, balls, strikeRate };
  ...
  getStrikeRate() {
    ...
    let sumStrikeRate = 0;
    this.innings.forEach((e) => {
      sumStrikeRate += e.strikeRate;
    });
  ...
  getProfile() {
    let totalRuns = this.innings.reduce((a, e) => {
      return (a += e.runs);
    }, 0);
    return {
      ...super.getProfile(),
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    let averageEconomy = this.spells.reduce((a, e) => {
      return (a += e.economy);
    }, 0);
    averageEconomy = averageEconomy / this.spells.length;
    return averageEconomy;
  }

  getProfile() {
    const totalWickets = this.spells.reduce((a, e) => {
      return (a += e.wickets);
    }, 0);
    return {
      ...super.getProfile(),
